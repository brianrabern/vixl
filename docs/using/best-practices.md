...
The plan tab header is built to lower cost. The buttons are Orchestrate, Build, and Build in new chat.
...
Build in new chat treats the plan as a hyper-tuned prompt. The next agent ingests only the plan, not the chat history. Use it when the planning thread is noise, or when you want a clean Agent-mode run that should not see the research turns.

Stay on the planning model if you want the cache. Click Build to continue in that chat. Click Orchestrate to keep the parent on that model and spawn sub-agents. Click Build in new chat when the plan file should be the whole prompt.
